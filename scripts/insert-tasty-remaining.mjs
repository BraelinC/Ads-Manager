// Insert remaining Tasty ads into Convex
...
async function main() {
  const file = '/home/braelin/.claude/projects/-home-braelin/23bfe3ff-bdd4-4d77-9b03-4f022f070d7c/tool-results/mcp-apify-get-actor-output-1774063829315.txt';
  const data = JSON.parse(readFileSync(file, 'utf8'));

  const items = data.items || [];
  const ads = items.map(item => transformAd(item));

  console.log(`Inserting ${ads.length} remaining Tasty ads...`);
